...
module DataStorage =
    ...
    let channels = Path.Join(Directory.GetCurrentDirectory(), "data", "channels.txt")
    ...
    let mutable suspectChannels: Set<uint64> =
        if not (File.Exists(channels)) then
            ...
            File.ReadLines (channels) |> Seq.map uint64 |> Set.ofSeq
    
    let addSuspectChannel (element: uint64) =
        suspectChannels <- Set.add element suspectChannels
        if not (File.Exists(channels)) then
            File.Create(channels) |> ignore
        let tokenizedChannels = suspectChannels
                                |> Set.toList
                                |> List.map (fun i -> $"%i{i}")
